...
| `refresh-reports.yml` | Lunes, miércoles y viernes 18:00 Chile | Re-investiga con Claude los **informes completos** de los 9 dinámicos (SPCX, BTC, HDSY, CLSK, MU, MSTR, CMPC, CENCOSUD, USD/CLP) y guarda un **snapshot de señales** para el backtest | Usa tu suscripción Claude |
| `on-demand-report.yml` | **Al instante, cuando lo pides** | Informe nuevo o actualización de cualquier activo: se dispara desde el botón de la app (abre una solicitud en GitHub) o desde Actions → Run workflow. Claude investiga, `scripts/add_asset.py` cablea el activo si es nuevo (símbolo, benchmark, gráfico, formato) y publica; avisa por Telegram | Usa tu suscripción Claude |
| `remove-asset.yml` | **Al instante, cuando lo pides** | Elimina un activo: botón 🗑 en la app (o Actions → Run workflow). `scripts/remove_asset.py` deshace todo el cableado y **archiva** su histórico en `data/history/_archive/`; reconstruye y publica | **Gratis** (sin IA) |
...
pero sus textos solo cambian cuando los pides desde la app (botón «↻ Actualizar informe»).
